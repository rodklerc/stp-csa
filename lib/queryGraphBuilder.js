...
QueryGraphBuilder.prototype._visit = function (graph, cluster, n, crossClusters,
  ...
  //      !!! These larger paths can be cyclic !!!
  //      Suppose two paths from n to the destination have been found:
  //      P1: n -> x1 -> x2 -> dest
  //      P2: n -> x3 -> dest
  //      Suppose the current path P is a -> x1 -> n. This path is clearly acyclic.
  //      If we add P1 to P, we will have a cyclic path. If we add P2 to P,
  //      we don't have a cycle. This means that some irrelevant trips may
  //      be extracted by the TripExtractor.
  //      At the expense of some extra computing time and space, one could
  //      store the query graph being built, in order to detect cycles. (But
  //      also note that a valid query graph *may* contain cycles!)
...
QueryGraphBuilder.prototype._flush = function (callback) {
  ...
  if (this._inverseClustering[this._s] == this._inverseClustering[this._t] &&
    ...
    if (this._borderStations[this._t]) {
      this._visit(this._sGraph, this._inverseClustering[this._s], this._t,
                  true, [this._s], [],
                  function (path) {
                    self.push(path);
                  });
